...
- **書き戻し範囲を変更セルに限定**。`Console`は変更したセルをその場で描き、
  ...
  全画面の書き戻しが残るのは、セル配列が画面の下でずれる場合——末尾スクロール、
  `clear`、全画面モードからの復帰——だけです。
...
CPUに書かせないことで、PPA（Pixel Processing Accelerator）や2D-DMAで
フレームバッファを塗ればキャッシュを経由せず、フルバーストで書けます。
計画は[`PPA_FILL_PLAN.md`](docs/PPA_FILL_PLAN.md)にまとめてあります（未着手）。
...
- `src/icm.rs`: システムAXIインターコネクトの調停優先度。表示DMAのPSRAM読み出しを最優先にしてDSI BridgeのFIFOアンダーラン（水色フラッシュ）を防ぐ
