...
- **toolbar**: ボタン3つ、セキュリティの南京錠（上表）、現在のアドレス
  （編集中はアドレス欄）、右端にWi-Fiのインジケーター
- **余白**: 8 px。ページの色で塗ります。無いと本文の1行目がアドレス欄の下端に
  接し、chromeと文書が同じ面のように見えます——見出しはglyphの箱が行の最上段
  から始まるので、実際に触れます。toolbarの再描画が塗るので、viewportの
  再描画が下から始まっても取り残されません
...
| 戻る `←` | 12 | 52 | 履歴が空なら灰色 |
| 進む `→` | 64 | 52 | 進む履歴が空なら灰色 |
| 再読込 `↻`／中止 `×` | 116 | 52 | 読み込み中は中止に変わる |
| 南京錠 | 180 | 24 | 押すと文言をstatus行へ |
| アドレス欄 | 212 | 1024 | 123セル |
| 全消去 `×` | 1204 | 32 | 欄の内側。**編集中だけ** |
| Wi-Fi | 1244 | 24 | 押すとWi-Fiメニューへ。当たり判定は画面右端まで |

当たり判定はどれもtoolbarの高さ全体（y=0..48）です。ここには他に押すものが
無く、帯の高さいっぱいの的かどうかが、指で押せるボタンとマウスが要るボタンの
差になります。

**48 pxはこのパネルで約4.1 mm**です（1280が約111 mmなので11.6 px/mm）。
指の的として推奨されるのは7 mm程度＝81 pxで、それは1本の帯に画面の高さの
8分の1を使うことになり、この画面では払えません。つまりこの数字は測って
決まるものではなく判断で、判断する方法は押してみることです。40 pxと48 pxの
両方をbuildして実機で押し比べ、**40は届く、48は楽**という結果で48にしました。
8 pxはviewportから出ていて、残りは640 pxです。

barの上のものはすべて`TOOLBAR_HEIGHT`から導いてあります（`BUTTON_WIDTH`も
`TOOLBAR_HEIGHT + 4`）。変えるときは1箇所で、導けているかどうかは`const`の
assertが見ます——欄がbarより高い、ボタンの絵がbarより高い、ボタンがアドレス欄を
食う、欄が編集できないほど狭い。

ボタンの絵はフォントのglyphを2倍角（16×32）で描いています。`←`（U+2190）、
`→`（U+2192）、`↻`（U+21BB）、`×`（U+00D7）はいずれも収録済みです。似た形の
`✕`（U+2715）や`⚠`（U+26A0）は**収録されていない**ので、使うと中空の四角に
なります（[FONT.md](FONT.md)）。

以前ここにあった「N links N lines」は削りました。初期のデバッグ用で、
アドレス欄が必要としていた208 pxを常時使っていたためです。読み込み中の受信KiBは
status行へ移しました。

アドレス欄は編集中だけ白い枠になり、右端に全消去の`×`が出ます。押すと欄を
空にして**開いたまま**にします。アドレスの末尾を直すのが普通の使い方なので
欄は現在のアドレスを保持したまま開きますが（下記）、全部打ち直したいときに
親指キーボードで1文字ずつ消すのがいちばん高くつくためです。中身が空のときは
灰色で、押しても何も起きないことが押す前に分かります。
...
| Wi-Fiのタップ | Wi-Fiメニューを開く。戻ると接続状態の文言がstatus行へ出る |
...
この自動再試行は接続管理器自身が再接続中の場合だけです。CLIの`wificonnect`は資格情報を
...
toolbarの右端に3本の棒があり、接続がどこまで進んだかを出します。

| 棒 | 状態 | 戻ったとき出る文 |
| --- | --- | --- |
| 0本＋斜線 | OFF、またはC6とのリンクが落ちている | `Wi-Fi is off`／`the link to the C6 is down` |
| 1本 | 接続中、再試行待ち、パスワード待ち、未接続 | `connecting`／`not connected: tap the Wi-Fi bars to choose a network`ほか |
| 2本 | associate済みだがアドレスが無い | `associated but with no address: leave and run ipconfig dhcp` |
| 3本 | associate済みでアドレスもある | `online` |

SSIDが分かっていてUTF-8であれば`on <SSID>`が付きます。UTF-8でないSSIDは
置換文字を並べるより落とします——文はそれ無しでも何が起きたか言っています。

**電波強度ではありません。**RSSIを取るのはC6へのRPC往復で、しかもこの画面で
訊かれている質問には答えません。ページが出ないときに知りたいのは「そもそも
ネットワークに乗っているか」です。棒は接続が進むにつれて増え、どこで止まって
いるかは開いたメニューと、戻ったときのstatus行が言葉で出します。

3本目が「アドレスもある」なのは、**ビューアが実際に取得を始める条件と
一致させる**ためです（`addressed_network`）。associateだけを見て満杯の棒を
出すと、`no-network`で失敗するページの上に3本が立ちます。

ここに置いてあるのは、ここが答えの要る場所で、かつシェルが動いていない場所
だからです。全画面モードの最中に`wifistatus`は打てないので、これが無いと
「リンクが死んでいる」と「サーバが死んでいる」の区別が付きません。

### タップするとWi-Fiメニューへ

棒を押すと`src/app/wifi_menu.rs`の全画面メニューが開き、Escapeで抜けると
ビューアが全面再描画されて戻ります。南京錠と違って**状態を言うだけでなく、
その状態を変えられる**入口です。報告だけにすると、一番手が出せない画面で
一番困っている状態を読み上げるだけのインジケーターになります。

戻り際に接続状態の文言をstatus行へ出すので、押して閉じるだけなら以前と同じ
「文を言う」動作になります。

取得中のページがあれば、メニューへ入る前にsocketを返して**Wi-Fi再接続待ち**
（`Source::WaitingForNetwork`）へ移します。接続すればIP stackが作り直され、
古いsocketはその時点で無効だからです。URLと履歴操作は保つので、メニューで
接続して戻ればそのページの取得が再開します。ローカルファイルの読み出しは
socketを持たずメニューが触るものも無いので、そのまま継続します。
...
| `src/app/browser.rs` | 画面、入力、履歴（戻る・進む）、アドレス編集、組み込みページ |
...
`Failure`と`Outcome`も共有しますが、中身は何も共有しません。ビューアの
ループから見ると、`Pending`が`Network`か`Local`かの違いだけで、進め方・
