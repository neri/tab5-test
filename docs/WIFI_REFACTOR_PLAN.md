...
## 状態: 完了（Stage 0〜7。Stage 8の長時間受入は実施しない）

**Stage 8（長時間・異常系の実機受入）はこの計画では実施しない**（利用者判断、
2026-09-04）。Stage 0〜7の範囲で実機に大きな問題が出ていないためである。問題が
出た時点で新しい計画を起票する。現状文書（[`WIFI.md`](WIFI.md)）は各Stageの実装時に
更新済みである。
...
| 8 | 長時間・異常系の実機受入と現状文書の更新 | 実施しない（2026-09-04、上記「状態」） |
