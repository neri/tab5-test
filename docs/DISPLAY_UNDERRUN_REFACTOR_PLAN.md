...
## 状態: **完了**（Stage 0〜4で全受入条件合格。Stage 5〜8は判断Aが合格したため実施しない）
...
> **実施しない。** 判断Aが200 MHz／DPI 80 MHzだけで合格し、Stage 4で本計画を完了に
> したため、Stage 5〜8は着手しなかった代替案である。以下は当時の検討内容を残したもので、
> 現在の実装仕様ではない。表示帯域の現状は[`DISPLAY_BANDWIDTH.md`](DISPLAY_BANDWIDTH.md)を
> 参照する。
