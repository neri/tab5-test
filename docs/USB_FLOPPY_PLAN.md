...
## 状態: **凍結**（Stage 1 直結実機確認済み／Stage 2 CBI ADSCで未解決／Stage 3〜4 未着手）

**再開の予定はない。** CBI ADSCがSETUP段階で失敗する原因は未特定のままだが、本計画は
ここで打ち切る。`src/usb/floppy.rs`は調査再開用のコードとして残してあるが、`usb.rs`から
読み込まず、レジストリもクラスドライバとして選択しないため、実行時の挙動には影響しない。
以下は凍結時点の記録で、当時のまま残してある。
