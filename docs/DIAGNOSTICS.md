...
FONT PSRAM: decoded bytes=1021279
FONT LZ4: compressed bytes=792089
FONT PSRAM: LZ4+CRC cycles=...
FONT ROM: ASCII bytes=3175
FONT PSRAM: UI address=0x48...
FONT SOURCE: ASCII plain DROM + A4 LZ4 -> PSRAM
LCD: D-PHY 4/4 ready
...
UI addressはPSRAM mapの`0x48000000..0x4a000000`内でなければなりません。A4のDROM blobはLZ4
containerで平文fontではなく、起動時検証に失敗した場合は`FONT PSRAM: allocation failed`または
`UI LZ4/validation failed`を出して停止します。ASCIIは非圧縮DROMです。
`font-drom-direct` featureの比較buildではこれらのfont install行の代わりに
`FONT SOURCE: ASCII + A4 plain DROM direct (A/B baseline)`だけが出ます。
