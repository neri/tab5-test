...
書き込むのは保存予定ディレクトリの`<名前>.part`で、完了してから本来の名前へ
改名します。`.part`も完成先と同じボリュームに置くので、将来ほかのファイル
システムを書き込み可能にしても確定時にファイルシステム間コピーは発生しません。したがって
...
RAMルート全体は書き込み可能です。カレントディレクトリが読み取り専用の`/vol`上なら、
ネットワークに触る前に断ります。
