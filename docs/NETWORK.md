...
| `httpget <host\|a.b.c.d>[:port] [path]` | 最小のHTTP/1.0 GET。ヘッダの先頭数行を表示し、本文をカレントディレクトリへ保存 |
...
`may_recv`が落ちることを終端の判定に使うためです。

### ヘッダと本文を分ける

`http::get`は**ヘッダの終わりの空行で切り分け**、ヘッダは保持し（上限
...
- ステータスコードは`Response::status`で**報告するだけ**で、この層は判断
  しません。404の本文を取っておく価値があるかは呼び出し側の問いで、本文を
  常にsinkへ渡すことで判断が1箇所にまとまります
