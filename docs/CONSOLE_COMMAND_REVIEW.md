...
削除・feature化の実施計画は[`COMMAND_RETIREMENT_PLAN.md`](plans/proposed/COMMAND_RETIREMENT_PLAN.md)です。
...
閉じたかどうかで決まります。この repo では`docs/plans/`以下の計画が作業単位なので、
...
紐づく計画の状態は各計画の`## 状態:`行から取りました（2026-09-04時点。計画の
置き場所を`docs/plans/`へ移す前の値です）。
