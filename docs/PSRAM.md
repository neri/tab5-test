...
allocator初期化直後、画面を開始する前にA4 UI font用の1,021,279 byteを永続確保します。DROMの
792,089 byteのLZ4 containerをこの最終bufferへ直接展開し、wrapper、圧縮block、展開後fontの長さ、
header、layout、CRCを検査します。従ってheap領域そのものは23,322,624 byteのままですが、通常アプリ
開始時点でA4 payloadとallocator管理分が使用済みです。展開用の別作業bufferは持たず、LZ4 matchは
同じ出力bufferの展開済み範囲を参照します。ASCII 1bpp fontは3,175 byteの非圧縮DROMを直接参照し、
PSRAMを使いません。比較用の`font-drom-direct` featureではA4の確保も行わず、平文A4をDROMから
直接参照します。以前の小さいA4 blobによる実機A/BではPSRAM版のBrowser最長viewport repaint値が
少し増えましたが、更新タイミングはDROM版とほぼ同じで表示にも問題がなく、利用者判断で許容されました。
日本語A4を加えた現構成の実機値は未確認です。
