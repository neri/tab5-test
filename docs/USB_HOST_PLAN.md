...
実機確認可能な単位でコミットする。[`SD_CARD_PLAN.md`](SD_CARD_PLAN.md)と
...
`app::run`のフレームループに乗せるポーリング方式へ書き直す必要がある
（後述）。
...
- 対象エンドポイント（Interrupt IN）へ周期的にトランザクションを発行し、
  ...
  して扱い、`app::run`のフレームループから毎フレーム（または
...
[`BOOT.md`](BOOT.md)のCPUクロック問題と同じく、実機でしか踏めない罠に
該当する可能性が高い。
...
[`BOOT.md`](BOOT.md)・[`SD_CARD_PLAN.md`](SD_CARD_PLAN.md)同様、以下は
「シミュレーションではなく実機でしか
...
長時間アイドル後、ハブの空きポートへの`GET_PORT_STATUS`が一時的に
`XCS_XACT_ERR`/STALLで失敗することがあった。SOF、VBUS、root portは維持され、
チャネル0を復旧してハブEP0へ新しいSETUPを入れると要求は再開した。したがって
通常の復旧ではバスリセットを行わない。

`Hub::port_status_quiet`は失敗時に、channel 0を停止・split設定解除・FIFO
フラッシュ・descriptor DMA再設定してから、ハブ自身宛ての無害な`GET_STATUS`を
SETUP再同期用に一度実行し、元の`GET_PORT_STATUS`を再試行する。成功時は無音である。
それでも失敗した走査は約1秒後の次の背景走査へ繰り延べ、成功した完全走査で失敗数を
リセットする。3回連続で回復できない場合だけ走査を停止し、`usbrescan`を案内する。
