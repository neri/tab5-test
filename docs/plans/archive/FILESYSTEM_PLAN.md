...
## 状態: 完了（Stage 1〜5、実機確認済み）

exFATのUnicode名と大きな連続ファイルの読み出しは**完了条件から外した**（利用者判断、
2026-09-04）。どちらもライブラリのプレビュー制限を突く経路で、失敗しても対処できず、
試験用の媒体を用意すること自体が難しい。Stage 5にメモとして残す。

壊れたimageの拒否をホストテストで確認する件は、以前から別タスクへ保留されている
（利用者判断、2026-08-24）。下の注記のとおり、**保留理由はすでに解消している**。ブロックデバイス層、MBR判定、RAMディスク（PSRAM固定8 MiB、起動時
...
**メモ: 確認していない読み出し経路**（完了条件ではない）

- **Unicode名の読み出し。** 上のプレビュー制限（断片化upcase、クラスタ境界をまたぐ
  entry set）が実際に問題になるとすればここである。
- **大きな連続ファイルの読み出し。**

どちらも完了条件から外した（利用者判断、2026-09-04）。理由は2つで、**判定がライブラリ
側の実装に依存するため問題が出ても対処できない**こと、そして**該当する媒体を用意する
のが難しい**ことである。実媒体で踏んだら、そのときライブラリ側の問題として扱う。

壊れたboot regionの拒否は、Stage 0から別タスクへ保留したホストテストの一部である
（上の「状態」）。
...
完了条件（当初）: 正常な実カードのexFAT媒体で大きな連続ファイルとUnicode名を読む。
壊れたboot regionの安全な拒否は、ローカル生成したimageを使うホストテストで確認する。
SD/USBはread-onlyを維持する。

**改訂（2026-09-04）:** 大きな連続ファイルとUnicode名の読み出しは完了条件から外し、
上のメモへ移した。壊れたboot regionの拒否は保留中のホストテスト側へ残す。実際の完了条件は
「実媒体のexFATをmountし、`mounts`が形式を表示し、`ls`が一覧・サイズ・更新日時を返し、
書き込みを拒否し、SD/USBがread-onlyのままであること」で、これは実機で確認済みである。
