...
実機確認可能な単位でコミットする。SDMMC/SDIOは「ホスト初期化」「カード
...
（[`BOOT.md`](../../BOOT.md)のCPUクロック問題と同種の「実機でしか踏めない罠」）。
...
### 追補: IDMACバッファのアラインメント（2026-08-24、ファイルシステム層追加時に発覚）

`fs`層のMBR判定を足したところ、`blkread sd0 0`はLBA 0を正しく読めるのに、同じ
`SdBlockDevice::read_blocks`を通る`devices`は512 byte全部がゼロを返した。CMD17も
DMAもエラーを出さず、`RINTSTS.DTO`も正常に立っていた。

原因はROMのキャッシュ操作関数が、64 byte境界から始まらない範囲を拒否すること。
`psram::writeback_invalidate`がその戻り値を捨てていたため、DMA後の無効化が実行されず、
CPUは自分がゼロ初期化したキャッシュの中身を読んでいた。スタック上の`[u8; 512]`は
Rust上のアラインメントが1なので、**呼び出し場所ごとに成否が変わる**。`sdread`／`sdmbr`／
`sdreadn`がこれまで動いていたのは、たまたまスタックスロットが整列していたからである。

この制約自体は`sdio.rs`のCMD53と`wifi/hosted.rs`（`#[repr(C, align(64))]`の
`DmaBuffer`）が最初から守っていた。SDカード側だけが契約を書いても守ってもいなかった。

対処は`sdmmc.rs`に閉じた。非整列バッファは整列済みステージング経由で転送し、
キャッシュ操作が拒否された場合は転送失敗として扱う。呼び出し側は変更していない。

**教訓**: DMAとキャッシュをまたぐ操作の戻り値を捨てない。捨てた場合の症状は
「エラーなしで転送前の内容が返る」であり、ゼロ初期化バッファではゼロが返るため、
一見「デバイスが何も返さなかった」ように見えて原因から遠ざかる。

### 追補: 1ブロック読み出しのコマンド（同上）

`read_blocks`は1ブロックでもCMD18＋ハードウェアauto-stopを使っていた。Stage 3の実機確認は
`sdreadn 0 4`であり、1ブロックのCMD18は一度も試されていない。1ブロックは`read_block`の
CMD17へ振り分けるようにし、`sdread`／`sdmbr`／`fs`層が同じ経路を通るようにした。
書き込み側はCMD24を実装しておらず、`sdwritetest`／`sdzero`がCMD25で実機受入済みなので
変更していない。
...
次のStageへ進む前に必ず実機でログを確認する（[`BOOT.md`](../../BOOT.md)のCPUクロック問題の
